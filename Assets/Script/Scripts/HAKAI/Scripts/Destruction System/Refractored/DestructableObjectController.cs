...
{
    public GameObject[] roots = new GameObject[17]; //とりあえず要素が17個入れられるようにした
    public DestroyedPieceController[] root_dest_pieces = new DestroyedPieceController[17];//要素が17個のピースコントローラー配列を作成
    ...
    private void Awake()//ゲームがスタートされるとすべての子オブジェクトたちにDestroyPiceControllerアタッチするためのwake
    {
        ...
        StartCoroutine(run_physics_steps(1));//コンポーネントを追加するルーチン 中身はDestroyPiceの58行目
    ...
    private void Update()
    {
        ...
        Debug.Log(destroyed_pieces);
        if (DestroyedPieceController.is_dirty) //DestroyPicecontrollerのis_dirtyがダメージが与えられていたら処理する

        {
            ...
            //全てのroot_dest_piecesの情報を読み取る
            for (int _i = 0; _i < 4; _i++)
                find_all_connected_pieces(root_dest_pieces[_i]);

            //root_dest_pieces配列内に格納された各DestroyedPieceControllerを起点としてfind_all_connected_pieces関数を呼び出して、接続された全てのピースを探索している
            ...
            foreach (var piece in destroyed_pieces) //destroyed_piecesの配列から一つ一つの要素を取り出してpiceに代入しています
            {
                ...
                if (piece.is_connected == false && piece.Cflg == true)
                {
                    ...
                    piece.Cflg = false;
                    Debug.Log(/*"崩れたオブジェクトの数"*/DestroyedPieceController.DestroyPieceCount);// ピースが崩れてピース同士の接続が切れたらカウントを増やす
    ...
    private void find_all_connected_pieces(DestroyedPieceController destroyed_piece)//DestroyedPieceControllerに接続されているピースのリストを取得してアタッチされているオブジェの起点に接続されているピースを探索するもの
    {
